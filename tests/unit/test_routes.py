...
class TestIndex:
    def test_renders_index_template(self) -> None:
        with flask_app.test_request_context("/"):
            with patch("app.app.render_template", return_value="<html>") as mock_render:
                app_module.index()
        mock_render.assert_called_once_with("index.html")


class TestHealth:
    def test_returns_200_with_ok_payload(self) -> None:
...
class TestListTodos:
    def test_query_orders_by_created_at_then_id_desc(self) -> None:
        mock_db = MagicMock()
        mock_db.execute.return_value.fetchall.return_value = []
        with flask_app.app_context():
            with patch("app.app.get_db", return_value=mock_db):
                app_module.list_todos()
        sql = mock_db.execute.call_args[0][0]
        assert "ORDER BY created_at DESC, id DESC" in sql

    def test_returns_serialized_rows(self) -> None:
...
class TestCreateTodo:
    def _mock_db(self, todo_row: dict[str, Any]) -> MagicMock:
    ...
    def test_inserts_stripped_text(self) -> None:
    ...
    def test_commits_after_insert(self) -> None:
        ...
        with flask_app.test_request_context("/api/todos", method="POST", json={"text": "Task"}):
            with patch("app.app.get_db", return_value=mock_db):
                app_module.create_todo()
        mock_db.commit.assert_called_once()

    def test_fetches_inserted_row_by_lastrowid(self) -> None:
    ...
    def test_empty_text_returns_400(self) -> None:
        with flask_app.test_request_context("/api/todos", method="POST", json={"text": ""}):
            response, status = app_module.create_todo()
        assert status == 400
        assert json.loads(response.get_data()) == {"error": "text is required"}

    def test_whitespace_only_text_returns_400(self) -> None:
        with flask_app.test_request_context("/api/todos", method="POST", json={"text": "   "}):
            response, status = app_module.create_todo()
        assert status == 400
        assert json.loads(response.get_data()) == {"error": "text is required"}

    def test_missing_text_key_returns_400(self) -> None:
        with flask_app.test_request_context("/api/todos", method="POST", json={}):
            response, status = app_module.create_todo()
        assert status == 400
        assert json.loads(response.get_data()) == {"error": "text is required"}
...
class TestToggleTodo:
    def _mock_db(self, result_row: dict[str, Any] | None) -> MagicMock:
    ...
    def test_update_sql_uses_not_done_expression(self) -> None:
    ...
    def test_passes_todo_id_to_update(self) -> None:
    ...
    def test_commits_after_update(self) -> None:
        mock_db = self._mock_db(_row(done=1))
        with flask_app.app_context():
            with patch("app.app.get_db", return_value=mock_db):
                app_module.toggle_todo(1)
    ...
    def test_returns_404_when_row_is_none(self) -> None:
...
class TestDeleteTodo:
    def test_delete_sql_targets_correct_id(self) -> None:
        mock_db = MagicMock()
        with flask_app.app_context():
            with patch("app.app.get_db", return_value=mock_db):
