...
class TestGetDb:
    ...
    def test_returns_same_connection_on_second_call(self):
        with flask_app.app_context():
...
class TestInitDb:
    def test_creates_todos_table(self):
        with flask_app.app_context():
            app_module.init_db()
            db = app_module.get_db()
            rows = db.execute(
