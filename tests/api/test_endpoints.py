...
class TestCreateTodo:
    ...
    def test_response_contains_todo_fields(self, client):
    ...
    def test_non_json_body_returns_400(self, client):
        resp = client.post(
            "/api/todos", data="not json", content_type="text/plain"
        )
        assert resp.status_code == 400
        assert resp.get_json() == {"error": "text is required"}
...
class TestToggleTodo:
    def _create(self, client, text="Test todo"):
        return client.post("/api/todos", json={"text": text}).get_json()["id"]

    def test_toggles_done_false_to_true(self, client):
        todo_id = self._create(client)
        data = client.patch(f"/api/todos/{todo_id}/toggle").get_json()
        assert data["done"] == 1

    def test_toggles_done_true_to_false(self, client):
        todo_id = self._create(client)
        client.patch(f"/api/todos/{todo_id}/toggle")  # → 1
        data = client.patch(f"/api/todos/{todo_id}/toggle").get_json()  # → 0
        assert data["done"] == 0

    def test_returns_200_with_todo_data(self, client):
...
class TestDeleteTodo:
    def _create(self, client, text="Delete me"):
        return client.post("/api/todos", json={"text": text}).get_json()["id"]
    ...
    def test_todo_is_removed_from_list(self, client):
        todo_id = self._create(client)
        client.delete(f"/api/todos/{todo_id}")
        todos = client.get("/api/todos").get_json()
        assert not any(t["id"] == todo_id for t in todos)

    def test_nonexistent_id_returns_204(self, client):
